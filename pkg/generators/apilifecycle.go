...
func parseAPILifecycle(comments []string) (map[string]string, []error) {
	...
	if commentTag != nil {
		...
		apiLifecycle[rules.KeyComponent] = commentValues[0]
...
func validateAPILifecycleKeys(keys map[string]string) []error {
	errors := []error{}
	for key, value := range keys {
		if !rules.AllowedTagKeys.Has(key) {
			errors = append(errors, fmt.Errorf("unrecognized key: %s. supported values are: %q", key, rules.AllowedTagKeys.List()))
		}
		if key == rules.KeyStatus && !rules.AllowedStatusNames.Has(value) {
...
func emitAPILifecycle(apiLifecycle map[string]string, g openAPITypeWriter) {
	...
	componentValue, exists := apiLifecycle[rules.KeyComponent]
	if !exists {
		return
	}
	g.Do("\"$.$\": map[string]interface{}{\n", componentValue)

	if statusValue, exists := apiLifecycle[rules.KeyStatus]; exists {
		g.Do("\"$.$\": ", rules.KeyStatus)
		g.Do("\"$.$\",\n", statusValue)
	}
	if minVersionValue, exists := apiLifecycle[rules.KeyMinVersion]; exists {
		g.Do("\"$.$\": ", rules.KeyMinVersion)
		g.Do("\"$.$\",\n", minVersionValue)
	}
	if featureGateValue, exists := apiLifecycle[rules.KeyFeatureGate]; exists {
		g.Do("\"$.$\": ", rules.KeyFeatureGate)
		g.Do("\"$.$\",\n", featureGateValue)
	}
	g.Do("},\n", nil)
}
